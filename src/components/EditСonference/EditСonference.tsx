...
import { Box, Button, Grid, FormControl, InputLabel, Input, InputAdornment, IconButton, TextField, FormHelperText, FormControlLabel, Checkbox, Tooltip, List, ListItem, ListItemText, ListItemIcon, ListItemAvatar, ListItemSecondaryAction, Avatar, ClickAwayListener } from '@material-ui/core';
...
const EditСonference: React.FC<Props> = (props) => {
    const match = useRouteMatch();
    let history = useHistory();
    //@ts-ignore
    const conferenceId = parseInt(match.params.conferenceId);
    const { sessionId, userAccountSuccess, conferenceUsers, conferenceSuccessAction, conferenceSuccess, setLoaderAction,
            clearConferenceUsersAction, searchUsersAction, isPublic, publicRoomPassword, isPublicAction, conferenceParticipantsAction, publicRoomPasswordAction } = props;
    ...
    useEffect(() => {
        if(sessionId) {
            ConferencesAPI.getConference(sessionId, conferenceId).then(data => {
                setTitle(data.conferenceName);
                setSelectedDate(data.startDate);
                setTimeStart(data.timeStart);
                setTimeEnd(data.timeEnd);
                conferenceParticipantsAction(data.conferenceParticipants);
                isPublicAction(data.isPublic);
                setConferencePublicKey(data.conferencePublicKey);
            })
        }
    }, [sessionId, conferenceId, conferenceParticipantsAction, isPublicAction]);
    ...
    return (
            ...
            <div className="grayBg newConference">
                ...
                <Box className="modalButtonGroup" mt={4}>
                    ...
                    <Button 
                        ...
                        disabled={title !== ''  ? false : true}>Сохранить конференцию</Button>
                </Box>                   
            </div>
            <AlertDialog 
                alertOpen={alertOpen}
        ...
        <div className="mobile-conference edit-mobile">
            ...
            <Button
                ...
                disableElevation 
                onClick={saveConference} 
                disabled={title !== ''  ? false : true}>Сохранить</Button>
        </div>
        ...
        <Notification 
            ...
            message="Вы успешно обновили конференцию!!!"
            severity="success" /> 
        </>
    )
}
...
const mapDispatchToProps = (dispatch: Dispatch<Action> & ThunkDispatch<any, any, AnyAction>) => ({
    ...
    conferenceParticipantsAction: (conferenceUsers: SearchUsers[]) => dispatch(actions.conferences.conferenceParticipantsAction(conferenceUsers)),
...
export default connect(mapStateToProps, mapDispatchToProps)(EditСonference);